...
public class AddJobForm extends LokiForm {
    ...
    public AddJobForm(MasterForm mHandle) {
        ...
        cbxAutoFileTransfer.setSelected(masterForm.getCfg().getAutoFileHandling());
    ...
    private void initComponents() {
        ...
        jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder("普通"));
        jPanel1.setName(""); // NOI18N

        cbxJobType.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Blender" }));
        cbxJobType.setToolTipText("选择任务类型");

        typeLabel.setText("类型:");

        nameLabel.setText("名称:");

        fileLabel.setText("项目文件:");

        firstFrameLabel.setText("第一帧:");

        lastFrameLabel.setText("最后一帧:");

        txtName.setToolTipText("输入一个唯一的任务名称");

        txtProjectFile.setToolTipText("指定项目文件的完整路径");

        txtFirstFrame.setToolTipText("将要渲染的帧的范围的第一帧(例如 '1')");

        txtLastFrame.setToolTipText("将要渲染的帧的范围的最后一帧(例如 '200')");

        FileBrowseButton.setText("浏览");
        ...
        jLabel1.setText("输出目录:");

        txtOutputDir.setToolTipText("渲染帧的输出目录");

        OutputBrowseButton.setText("浏览");
        ...
        jLabel2.setText("输出文件前缀:");

        txtOutputPrefix.setToolTipText("为渲染的帧指定一个文件前缀(例如 \"scene1_\" 将会生成 \"scene1_0024.jpg\", 等)");
        ...
        cbxAutoFileTransfer.setText("启用文件自动传输");
        cbxAutoFileTransfer.setToolTipText("无需检查此项如果你为项目文件设置了一个所有节点都可访问的网络共享.\n对于含有很多贴图的大项目是很有用的, 等.");
        ...
        pnlTileRendering.setBorder(javax.swing.BorderFactory.createTitledBorder("分块渲染"));
        pnlTileRendering.setToolTipText("分块渲染会将一帧分割为几部分进行块的平行渲染.");

        cbxEnableTile.setText("启用");
        ...
        comboboxTileMultiplier.setToolTipText("选择一个因子来指定分块的大小");
        ...
        lblMultiplier.setToolTipText("分块渲染的总数");
    ...
    /**
     * performs final validation then creates a new job object with the given
     * values and passes it to the masterForm
     *
     * @param evt
     */
    private void saveButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveButtonActionPerformed
        ...
        if (txtName.getText().equals("")) {
            JOptionPane.showMessageDialog(rootPane, "请输入一个名称.");
            valid = false;
        ...
        try {
            ...
            JOptionPane.showMessageDialog(rootPane, "请输入一个合法的项目文件.");
            valid = false;
        ...
        try {
            ...
            if ((first > last) && valid) {
                JOptionPane.showMessageDialog(rootPane, "最后一帧 "
                        + "必须和第一帧一样或者比它大.");
                valid = false;
                txtFirstFrame.requestFocusInWindow();
            }
            if (first < 1 && valid) {
                JOptionPane.showMessageDialog(rootPane, "帧数必须是 "
                        + "正数.");
                valid = false;
                txtFirstFrame.requestFocusInWindow();
            }
            ...
            if (valid) {
                JOptionPane.showMessageDialog(rootPane, "请输入合法的 "
                        + "第一帧和最后一帧数值.");
                valid = false;
                txtFirstFrame.requestFocusInWindow();
            }
    ...
    private void FileBrowseButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_FileBrowseButtonActionPerformed
        ...
        fileChooser.setDialogTitle("选择一个项目文件");
        fileChooser.addChoosableFileFilter(new BlendFilter());
        if (fileChooser.showDialog(jLabel1, "选择") == JFileChooser.APPROVE_OPTION) {
    ...
    private void updateMultiplierText() {
        ...
        String txt = multiplier.toString() + " * " + multiplier.toString()
                + " = " + Integer.toString(multiplier * multiplier) + " 块";
    ...
    private void OutputBrowseButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_OutputBrowseButtonActionPerformed
        ...
        dirChooser.setDialogTitle("选择一个输出目录");
        dirChooser.setFileSelectionMode(JFileChooser.DIRECTORIES_ONLY);
        dirChooser.setAcceptAllFileFilterUsed(false);
        dirChooser.setCurrentDirectory(masterForm.getCfg().getOutDirFile());
        if (dirChooser.showDialog(jLabel1, "选择") == JFileChooser.APPROVE_OPTION) {
